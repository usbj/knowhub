...
/**
 * 项目全局开关读取收口。
 * 全项目唯一"知道项目审核开关从哪来"的地方：ProjectServiceImpl.publish 只调
 * {@link #isReviewEnabled()}，业务侧不直接使用 SysConfigUtil。
 *
 * 当前实现走系统设置 sys_config['knowhub.project.review_enabled']（BOOLEAN 类型，
 * 见 sql/knowhub-project.sql，默认 true 开启审核）。SysConfigUtil 只读 Redis 永久缓存，
 * 编辑设置项时由 SysConfigServiceImpl 重写缓存，运行时生效无需重启。
 * 将来若再换存储，仅需改本方法内部实现，签名与调用方零改动。
 * 结构与 {@link BlogConfigReader} / {@link ResourceConfigReader} 同构。
 */
@Component
public class ProjectConfigReader {

    /** 系统设置键：项目审核开关（BOOLEAN，true 开启审核 / false 直通发布） */
    public static final String CONFIG_KEY_REVIEW_ENABLED = "knowhub.project.review_enabled";
