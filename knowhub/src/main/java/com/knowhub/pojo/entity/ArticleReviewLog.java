...
/**
 * 文章系统审核流水实体，对应 article_review_log 表。
 * 结构与 blog_review_log / resource_review_log / project_review_log 完全同构：只追加不改不删，
 * 记全量审核历史；主表只存状态机当前值，流水记全量轨迹。只记动作不记状态前后
 * (action 隐含转移语义)。
 *
 * 字段说明：
 * - action 见 {@link com.knowhub.enums.ReviewAction}（代码层枚举复用，字典 review_action
 *   博客/资源/项目/文章共用 dict_id=25），隐含文章状态转移语义
...
public class ArticleReviewLog {
    ...
    /** 操作人昵称（非表字段，由 listByArticleId left join sys_user 带出，供 VO 直接展示） */
    private String operatorNickname;

    public ArticleReviewLog() {
    }

    public ArticleReviewLog(Long articleId, String action, Long operatorId, String operator, String role, String advice) {
