 ...
 * 调度器按 cron 驱动（初始 cron 0 10 0 1 * *，每月 1 日 00:10，与原 @Scheduled cron 同款 6 段式），
...
public class AuditMonthlyReportTask {
    ...
    /**
     * 生成月报。由 sys_job 调度器按 cron 0 10 0 1 * * 触发（每月 1 日 00:10，无参方法，符合 rookie findTaskMethod 要求）。
     * 注意 Spring cron 表达式为 6 段（含秒），与 Unix cron 5 段不同。
     */
