/**
 * 系统设置 store —— knowhub 前台
 * ------------------------------------------------------------------
 * 照搬后台 rookie-ui/src/stores/system-config.ts，仅改 localStorage key 前缀为 knowhub-。
 * 与字典 store 的「登录后全量预加载」不同：系统设置可能含不宜整体暴露的关键信息，
 * 故按 `configKey` 单项异步拉取，内存只缓存「已请求过的 key 的值」，
...
export const SYS_CONFIG_CACHE_STORAGE_KEY = 'knowhub-system-config-cache'

type SysConfigValueMap = Record<string, string | null>

/**
 * 读取本地缓存的系统设置值，避免刷新后对已知 key 重复请求。
 */
