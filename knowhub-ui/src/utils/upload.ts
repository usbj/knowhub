/**
 * 文件作用：
 * 封装文件上传流程（申请令牌 → PUT 上传 → 确认 → 取稳定解析引用），供博客封面上传、
 * MD 编辑器正文插图等场景复用。照搬后台 rookie-ui 的 utils/upload.ts，逻辑不变。
 * 关键约定：
 ...
 * - 这是 knowhub-ui 复刻的新增工具，不修改任何既有 utils 文件。
 *   与后台 rookie-ui 版本的唯一差异：Token 头取自前台 user store 的 knowhub-user-token（后台是 rookie-user-token）。
 */
 ...
 * - 非 2xx 状态码抛错，错误信息包含 OSS 返回体便于排查。
 * 参数：
...
/**
 * 方法效果：
 * 执行完整的预签名直传三步流程：申请令牌 → PUT 直传 OSS → 确认。
 * 数据流转：
 ...
 * - 4. access=PUBLIC 时回填 publicUrl(/file/resolve/{objectId})供 <img> 直引。
 * 参数：
 * - `options`：预签名直传选项（file / businessType / access / bizRefId / onProgress）。
 * 返回值：
 * - 上传结果（objectId / publicUrl / objectKey）。
 * 副作用：
 * - 任意一步失败即抛错；直传成功但 confirm 失败时，对象已在 OSS 但元数据状态异常，
...
export const presignedUploadFlow = async (
  ...
  // 2. PUT 直传 OSS：用原生 XHR 支持进度回调；中转模式带 Token，直链模式不带（预签名自带鉴权）
